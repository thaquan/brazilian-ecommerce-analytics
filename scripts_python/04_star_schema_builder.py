...
=============================================================================
 Goal : Transform the normalized OLTP tables (8 tables) into an optimized
        ...
        1 Fact Table   → Fact_Order_Items (the core business events)
        5 Dim Tables   → Dim_Date, Dim_Customers, Dim_Products,
                          Dim_Sellers, Dim_Reviews
...
# ==========================================================================
# 6. BUILD FACT_ORDER_ITEMS — THE CENTRAL FACT TABLE
# ==========================================================================
...
# ── JOIN 3: Reviews → brings in review_score ──
# Use LEFT JOIN because some orders might not have reviews
fact = fact.merge(
    reviews[['order_id', 'review_score']],
    on='order_id', how='left'
)

# ── JOIN 4: Payments → aggregate total payment per order ──
# Since payments can have multiple rows per order (multiple payment methods),
# we aggregate FIRST, then join the totals
payment_agg = payments.groupby('order_id').agg(
    total_payment=('payment_value', 'sum'),
    payment_installments_max=('payment_installments', 'max'),
    payment_type_primary=('payment_type', 'first')  # Most common payment method
).reset_index()
...
# Was the delivery late? Boolean flag for easy aggregation
...
# ==========================================================================
# 7. EXPORT TO CSV (for Power BI import)
# ==========================================================================
print("\n" + "=" * 80)
...
# ==========================================================================
# 8. ALSO LOAD INTO SQL SERVER (for Power BI DirectQuery option)
# ==========================================================================
print("\n" + "=" * 80)
...
print("""
  STAR SCHEMA ARCHITECTURE:
                    ...
                    Dim_Date
                       ...
                       | order_date_key
                       |
  Dim_Customers ---[customer_unique_id]--- FACT_ORDER_ITEMS ---[product_id]--- Dim_Products
                                                |
                                           [seller_id]
                                                |
                                           Dim_Sellers

  Relationship Type: All 1-to-Many (Dim -> Fact)
  Filter Direction:  Single (Dim -> Fact only)
""")
...
print("\n  Fact table measures available:")
measures = ['price', 'freight_value', 'line_total', 'total_payment',
            'delivery_days', 'delivery_delay_days', 'is_late_delivery', 'review_score']
for m in measures:
    dtype = fact_final[m].dtype
    non_null = fact_final[m].notna().sum()
    print(f"    {m:<30} type={str(dtype):<12} non-null={non_null:>10,}")
...
print("  Ready to import into Power BI Desktop!\n")
